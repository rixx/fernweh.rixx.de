...
def post(review, number, in_reply_to=None, dry_run=False):
    ...
    if len(text) > (280 - 1 - 23):  # URLs are always 23 chars
        raise Exception(f"tl;dr too long in {review.id}: {len(text)} > {280 - 1 - 23}")
...
def post_next(dry_run=False):
    ...
    reviews = sorted(
        ...
        key=lambda x: (
            ...
            x.metadata.get("social", {}).get("twitter", {}).get("id")
            or 0,  # We use the fact that Twitter has monotonic IDs
            x.relevant_date,
        ),
    )
